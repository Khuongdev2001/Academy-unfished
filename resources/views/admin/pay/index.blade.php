    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Danh Sách Thanh Toán {{ $option  ?? "" ? "(Thùng Rác)" :"" }} </h1>
    ...
    <div id="content" class="p-2">
        <div class="block block-rounded">
            ...
            <div class="actions user action-all px-4 pt-4">
                ...
                <a href="{{ route("admin.pay") }}" class="btn-action btn-dual btn-current mr-2 ">
                ...
                <a href="{{ route("admin.pay","trash") }}" class="btn-action btn-danger btn-trash mx-2">
            ...
            <div class="block-content">
                <form id="DataTables_Table_0_wrapper" class="sort_table dataTables_wrapper dt-bootstrap4 no-footer">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-6">
                            <input type="text" name="seach" id="seach" class="form-control form-control-sm" placeholder="Tìm kiếm...." value="{{request("seach")}}">
                        </div>
                            ...
                            <table class="text-center table w-100 table-bordered table-vcenter js-dataTable-full datable no-footer display nowrap dataTable dtr-inline collapsed" id="example" role="grid">
                                <thead>
                                    <tr id="header-row">
                                        ...
                                        <th class="sorting sort" style="width: 5%;">
                                            Tên Thanh Toán
                                        ...
                                        <th class="" style="width: 5%;">Tác Vụ</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @php
                                    ...
                                    <tr>
                                        <td>
                                            <div class="custom-control-success">
                                                <div class="custom-control custom-checkbox"><input type="checkbox" pay="{{$item->id}}" class="custom-control-input multitask" id="user-4"><label class="custom-control-label" for="user-4"></label></div>
                                            </div>
                                        </td>
                                        <td>
                                           {{++ $temp}}
                                        ...
                                        @if(empty($option))
                                            <td>
                                                <a href="{{ route('admin.pay.update', $item->id) }}" class="btn-update btn btn-sm btn-success" ><i class="fas fa-pen"></i></a>
                                                <a href="{{ route('admin.pay.delete', $item->id) }}" class="btn-delete btn btn-sm btn-danger ml-1"><i class="fas fa-trash-alt"></i></a>
                                            </td>
                                            @else
                                            <td>
                                                <a href="{{ route('admin.pay.restore', $item->id) }}" class="btn-restore btn btn-sm btn-success" user="4"><i class="fas fa-trash-restore"></i></a>
                                                <a href="{{ route('admin.pay.destroy', $item->id) }}" class="btn-destroy btn btn-sm btn-danger ml-1" user="4"><i class="fas fa-trash-alt"></i></a>
                                            </td>
                                        @endif
                                    </tr>
                                    @endforeach
                        ...
                        <div class="box-paginate col-12">
                            <ul class="paginate list-unstyled">
                                @for ($i = 1; $i <= ceil($pays->total() / $pays->perPage()); $i++)
                                    <li>
                                        <button name="page" value="{{ $i }}" class="@if($pays->currentPage()==$i) active @endif">{{ $i }}</button>
                                    </li>
                                @endfor
                            </ul>
                        </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" id="registermodal">
            ...
            <div class="modal-body">
                <h4 class="modal-header-title">{{ isset($pay) ? "Cập nhật Thanh Toán":"Thêm Thanh Toán" }}</h4>
                <form method="POST" action="{{ isset($pay)? route('admin.pay.update',$pay->id) : route('admin.pay.add') }}" enctype="multipart/form-data" class="slider-add slider-update row">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <input type="text" name="name" id="name" class="form-control form-control @error('name') is-invalid @enderror" value="{{old("name") ??$pay->name ??''}}">
                        @error('name')
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <input type="text" name="discount" placeholder="" id="discount" class="form-control form-control @error('discount') is-invalid @enderror" value="{{old("discount") ??$pay->discount ??''}}" >
                        @error('discount')
                            <small class="error">{{$message}}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-12">
                        <label for="info" class="label-Unaffected" >Thông tin chuyển khoản : *</label>
                        <textarea name="info" id="info" placeholder="Nội dung bài viết" class="form-control @error('info') is-invalid @enderror" cols="30" rows="5">{{old("info") ??$pay->info??''}}</textarea>
                        @error('info')
                            <small class="error">{{$message}}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-12 position-relative">
                        ...
                        <div class="box-upload position-relative my-2 @error('thumbnail') is-invalid @enderror">
                            <img src="{{asset($pay->thumbnail ?? "" )}}" id="img-uploaded" class="img-responsive img-circle" />
                    ...
                    <div class="form-group col-12 ">
                        ...
                        <a href="{{route("admin.pay")}}" class="btn btn-hero-sm btn-hero-info float-right"><i class="fas fa-save"></i> Chuyển trang thêm</a>
                    </div>
                </form>
            </div>
        </div>
